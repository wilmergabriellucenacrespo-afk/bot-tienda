// ESCUDOS GLOBALES ANTI-CRASH (Evita que Render apague el bot por errores de red)
...
// Iniciar el agente de forma segura
...
botTienda.action(/pagar_(.+)/, async (ctx) => {
  ...
  if(!compra) return ctx.editMessageText("Tiempo de sesión agotado. Vuelve al menú.", { reply_markup: { inline_keyboard: [[{ text: "🏠 Menú", callback_data: "menu_inicio" }]] }});
  ...
  // Búsqueda segura del ID de servicio
...
botTienda.on('photo', async (ctx) => {
  ...
  await ctx.reply('✅ *Comprobante enviado exitosamente*\n\nEl departamento de administración está verificando tu pago.', {
    parse_mode: 'Markdown',
    reply_markup: {
      inline_keyboard: [
...
botAdmin.action('admin_tasas', async (ctx) => {
  await ctx.answerCbQuery('Consultando API...').catch(()=>{});
...
botAdmin.on('text', async (ctx, next) => {
  if (ctx.from.id === MI_ID) {
    ...
    if (texto.startsWith('TASA ')) {
      ...
      return ctx.reply('❌ Formato incorrecto. Ejemplo correcto: TASA BCV 36.60').catch(()=>{});
    }
  }
  return next();
});
...
botAdmin.action('admin_ganancias', async (ctx) => {
  await ctx.answerCbQuery().catch(()=>{});
  await ctx.reply('💰 *Módulo de Ganancias*\n\nListo para integrar con Firebase.', { parse_mode: 'Markdown' }).catch(()=>{});
});
...
botAdmin.action(/rechazar_(.+)/, async (ctx) => {
  ...
  await botTienda.telegram.sendMessage(orden.userId, '❌ *Pago Rechazado*\n\nHemos encontrado un problema al verificar tu comprobante. Contacta a soporte.', { parse_mode: 'Markdown' }).catch(()=>{});
  await ctx.editMessageCaption(`❌ *PAGO RECHAZADO* (Orden #${ordenId})`, { parse_mode: 'Markdown' }).catch(()=>{});
  pagosPendientes.delete(ordenId);
});

botTienda.launch().catch(err => console.log("Error al iniciar Tienda:", err.message));
botAdmin.launch().catch(err => console.log("Error al iniciar Admin:", err.message));
console.log("¡Sistema blindado arrancado con éxito!");
